...
:Compile
  ...
  echo.
  echo Running Maven to build and push the webapp / env is %envName%
  CALL mvn -Djib.to.auth.username=AWS -Djib.to.auth.password=%pass1%%pass2% -Djib.from.auth.username=AWS -Djib.from.auth.password=%pass1%%pass2% -Daws.env.name=%envName% clean package -Pprod -PbuildAndPushImage | "C:\Program Files\Git\usr\bin\tee" %TEMP%\maven-output.txt
...
:Compile_failed
  echo.
  echo.
  echo Failed to build and push your build.
GOTO Cleanup
...
:Cleanup
  echo del /q %TEMP%\ecr_pass.txt      > nul 2>&1
  echo del /q %TEMP%\maven-output.txt  > nul 2>&1
