...
:Compile
  ...
  echo.
  echo Running Maven to build and push the Sync Service / env is %envName%
  CALL mvn -Djib.to.auth.username=AWS -Djib.to.auth.password=%pass1%%pass2% -Djib.from.auth.username=AWS -Djib.from.auth.password=%pass1%%pass2% -Daws.env.name=%envName% clean package -PbuildAndPushSyncService | "C:\Program Files\Git\usr\bin\tee"  %TEMP%\maven-output.txt
