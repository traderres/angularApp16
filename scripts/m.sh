...
#   3. Run the mvn command with these profiles:  "prod" and "buildAndPushImage"
...
# Run the maven profile to build and deploy the image to Amazon's ECR
echo -e "Running Maven with these profiles:  prod and buildAndPushSyncService  env is $1"
echo -e "\tmvn -Djib.to.auth.username=AWS -Djib.to.auth.password=\$PASSWORD -Djib.from.auth.username=AWS -Djib.from.auth.password=\$PASSWORD -Daws.env.name=$1  clean package -PbuildAndPushImage\n"
mvn -Djib.to.auth.username=AWS -Djib.to.auth.password=$PASSWORD -Djib.from.auth.username=AWS -Djib.from.auth.password=$PASSWORD -Daws.env.name=$1  clean package -PbuildAndPushImage | tee /tmp/output.txt

# Check if the build worked successfully
